(function () {
  ...
  const PAGES = {
    index: (t, nav, footer, lang) => `${nav}
      <section class="hero">
        ...
        <img class="hero-image" src="/public/crown-chain-social.png" alt="${CROWN_TEXT[lang].preview}">
      </section>
    ...
    games: (t, nav, footer, lang) => `${nav}
      ...
      <section class="grid" aria-label="${t.common.games}">
        <article class="card">
          <img class="game-thumb" src="/public/crown-chain-social.png" alt="${CROWN_TEXT[lang].preview}">
          <h2>Crown Chain</h2>
          <p>${CROWN_TEXT[lang].body}</p>
  ...
  const TEXT = {
    ...
    ko: {
      ...
      privacy: { lead: "게임은 계정 없이 플레이할 수 있습니다.", playerTitle: "사용자 입력", player: "랭킹 제출 시 게임 이름, 표시 이름, 점수, 보드 크기, 언어, 모드, 종료 방식, 제출 시간이 저장될 수 있습니다. 단어 제안에는 제안 단어와 언어 정보가 포함될 수 있습니다.", analyticsTitle: "분석", analytics: "Cloudflare Web Analytics와 집계된 게임 이벤트를 사용할 수 있습니다. 전체 입력 단어 목록은 이벤트에 포함하지 않습니다.", adsTitle: "광고", ads: "Google AdSense가 활성화되면 Google 및 파트너가 정책에 따라 쿠키 등을 사용할 수 있습니다." },
      contact: { lead: "버그, 단어 문제, 랭킹 문제, 정책 관련 문의는 GitHub로 알려주세요.", body: "누락 단어는 언어 정보가 함께 전달되는 게임 내 제안 버튼을 사용하는 편이 좋습니다. 랭킹 문제는 가능하면 게임 이름과 모드를 함께 적어 주세요.", button: "GitHub 이슈 열기" },
    ...
    ja: {
      ...
      privacy: { lead: "ゲームはアカウントなしで遊べます。", playerTitle: "プレイヤー入力", player: "ランキング送信時にゲーム名、表示名、得点、盤面サイズ、言語、モード、終了方法、送信時刻が保存される場合があります。単語提案には提案語と言語情報が含まれます。", analyticsTitle: "分析", analytics: "Cloudflare Web Analytics と集計されたゲームイベントを利用する場合があります。入力した全単語の一覧はイベントに含めません。", adsTitle: "広告", ads: "Google AdSense を有効にした場合、Google とパートナーがポリシーに従って Cookie などを使用する場合があります。" },
      contact: { lead: "不具合、単語、ランキング、ポリシーに関する連絡は GitHub で受け付けます。", body: "不足語は、言語情報も送れるゲーム内の提案ボタンを使うのがおすすめです。ランキングの問題は、できればゲーム名とモードも添えてください。", button: "GitHub issue を開く" },
  ...
  const CROWN_TEXT = {
    en: {
      ...
      note: "Basic and Chaos have separate leaderboards, and best scores are also tracked separately."
    },
    ko: {
      preview: "크라운 체인 보드 미리보기",
      body: "말을 잡을 때마다 현재 말이 바뀌는 짧은 체스 연쇄 퍼즐입니다.",
      support: "표준 말만 나오는 기본 모드와 레벨이 오르며 변형 말이 추가되는 카오스 모드를 제공합니다.",
      note: "기본과 카오스는 리더보드가 따로 나뉘며, 최고 점수도 별도로 기록됩니다."
    },
    ja: {
